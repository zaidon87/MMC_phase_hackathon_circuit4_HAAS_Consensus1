# Local Consensus Correction v1

## Purpose

This document proposes the next professional development step for the neighbor-consensus MMC controller.

The current model uses immediate neighbor capacitor voltages. The next step is to generalize this into a scalable **Local Consensus correction** that works for arbitrary `N` and avoids hardcoded submodule counts.
